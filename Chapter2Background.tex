\chapter{Background} \label{sec:background}
        ...
        \subsection{Combined PET/CT} \label{sec:combined_pet_ct}
            ...
            \subsubsection{Attenuation Correction} \label{sec:attenuation_correction}
                ...
                \begin{figure}
                    ...
                    \includegraphics[width=1.0\linewidth]{figures/background_nac_tof_example.png}
                    
                    \captionsetup{singlelinecheck=false, justification=raggedright}
                    \caption{This figure shows an example of simulated \gls{NAC} \gls{TOF} data, with no noise, of the thorax with a spherical lesion in the lungs.} \label{fig:combined_pet_ct_nac_tof_example}
                \end{figure}
                
                \begin{figure}
                    \centering
                    
                    \includegraphics[width=1.0\linewidth]{figures/background_ac_tof_example.png}
                    
                    \captionsetup{singlelinecheck=false, justification=raggedright}
                    \caption{This figure shows an example of simulated \gls{AC} \gls{TOF} data, with no noise, of the thorax with a spherical lesion in the lungs.} \label{fig:combined_pet_ct_ac_tof_example}
                \end{figure}
    ...
    \longsection{Inverse Problems and Optimisation}{sec:inverse_problems_and_optimisation}
        ...
        \subsection{Introduction to Inverse Problems and Optimisation} \label{sec:introduction_to_inverse_problems_and_optimisation}
            ...
            Because it is not usually possible to directly invert to find the solution, because of the size of the problem for instance, then the solution must be optimised for. In order to attempt to find the solution to an inverse problem there are two things which are required; first the forward operator is required which is the inverse of the inverse problem, which is being solved, or the forward problem, second, at least, a model of the noise present in the system is required.
        ...
        \subsection{Objective Function} \label{sec:objective_function}
            ...
            An example of an objective function would be \gls{MAE}. For a vector of some values \gls{MAE} subtracts the estimated value from the true value, finds the absolute of this so all negative values are multiplied by $-1.0$ (this is because the error should be the distance to the true value regardless of if the estimated value is greater or less than the true value) and then sums together this value for all values in the vector and divides by the number of values in the vector. This method finds the mean error over all estimated values. If the estimated values approach the true values then the value of the \gls{MAE} will approach zero in a linear fashion. If \gls{MSE} is used then rather than taking the absolute of the error the square is taken instead, taking the square causes the error to increase exponentially as it becomes larger. \gls{MAE} and \gls{MSE} are usually used in regression like problems where a line is fit though a data set.
        ...
        \subsection{Optimiser} \label{sec:optimiser}
            The optimiser takes an initial estimate of the values that are to be optimised and at least a function to either minimise or maximise the output of with the input of the estimate. The method by which the optimiser goes about updating the value of the estimate is what differentiates most optimisation algorithms.
            
            \gls{GD} is a commonly used optimisation algorithm. \gls{GD} attempts to find the gradient of the objective function over time and takes steps, of a given size, through the plane of the estimate in the direction calculated from the gradient which most shrinks the value of the objective function. The step size of \gls{GD} can be set to a value or found using a line search which optimises the step size. Momentum can be used in \gls{GD} where the current update direction is linear combination, with a predefined weighting, of the current gradient and the previous update direction. Momentum attempts to reduce the effect of local minimum solutions or non-convex solution spaces by reducing the likelihood of the current update causing the optimisation to jump back and forth between two close values. 
    ...
    \longsection{PET Image Reconstruction}{sec:pet_image_reconstruction}
        ...
        \subsection{Analytic Image Reconstruction} \label{sec:analytic_image_reconstruction}
            ...
            \begin{figure}
                ...
                \captionsetup{singlelinecheck=false, justification=raggedright}
                \caption{This figure shows an example of a simulated \gls{TOF} \gls{FBP} reconstruction, with noise, of the thorax with a spherical lesion in the lungs.}
                \label{fig:analytic_image_reconstruction_fbp_example}
            \end{figure}
            
            An analytic solution is one which offers a direct mathematical method through which one thing can be transformed to another. All analytical solutions can be proven. An analytical solution for \gls{PET} reconstruction is \gls{BP}, in \gls{2D} \gls{BP} is the Radon transform. To apply the Radon transform to a \gls{2D} sinogram; first the rows of the sinogram, corresponding to $\SI{0}{^{\circ}}$ through $\SI{180}{^{\circ}}$, are taken individually and have the Fourier transform applied to them. Then the result of this is reshaped to polar coordinates so that each row is the diameter of a circle and a \gls{2D} Fourier transform is applied giving the final output image.
            
            \gls{BP} gives a low quality image but is computationally fast. \gls{BP} will usually result in there being numerous streak like artefacts in the output image, the artefacts are exacerbated as noise increases. Artefacts can be caused because \gls{BP} does not account for the stochastic nature of the acquisition process, neither does \gls{BP} account for other factors such as positron range as discussed in~\Fref{sec:attenuation}.
            
            \gls{FBP} was proposed as a solution to some of the issues apparent in \gls{BP}. In \gls{FBP} a ramp filter is applied to the sinogram before it is Fourier transformed, thus removing some high frequency information or noise, an example of a \gls{FBP} reconstruction can be seen in~\Fref{fig:analytic_image_reconstruction_fbp_example}. Because of its speed and good quantitative results \gls{FBP} was the gold standard of \gls{PET} image reconstruction, however because of the issues mentions above it has now almost entirely been replaced by iterative reconstruction methods. \gls{FBP} is now almost solely used in longitudinal studies which started before the use of iterative reconstruction methods became prevalent~\boxcite{FBPReviewBib, PETCTReviewBib}.
