    ...
    \longsection{PET}{sec:pet}
        ...
        \subsection{The Physics of PET} \label{sec:the_physics_of_pet}
            ...
            \subsubsection{Static and Dynamic Acquisition} \label{sec:static_and_dynamic_acquisition}
                ...
                \begin{itemize}
                    \item The first and most common type of \gls{PET} scan is a static \gls{PET} scan. The patient is scanned only when the injected radiotracer has distributed through their body and eventually approximately stabilised. %KT ``perfused'' is wrong. I'd say''distributed''. ``consistent'' sounds weird, so I'd say ``eventually approximately stabilised'' %ACW done, thank you
...
%                \end{itemize}
            ...
            \subsubsection{Time of Flight PET} \label{sec:time_of_flight_pet}
                ...
                \begin{figure}
                    ...
                    \captionsetup{singlelinecheck=false, justification=raggedright}
                    \caption{Example of some \glsed{NAC} \gls{NTOF} data, with noise, with no motion, randoms or scatters, of the thorax with a spherical lesion in the lungs. Coronal view.} \label{fig:time_of_flight_pet_non_tof_example}
                \end{figure}
                ...
                \begin{figure}
                    ...
                    \captionsetup{singlelinecheck=false, justification=raggedright}
                    \caption{Example of some \glsed{NAC} \gls{TOF} data, with noise, with no motion, randoms or scatters, of the thorax with a spherical lesion in the lungs. Coronal view.} \label{fig:time_of_flight_pet_tof_example}
                \end{figure}
                ...
                An example of some \glsed{NAC} \gls{NTOF} and \glsed{NAC} \gls{TOF} data can be seen in~\Fref{fig:time_of_flight_pet_non_tof_example} and~\Fref{fig:time_of_flight_pet_tof_example} respectively, notice the difference in distribution of counts in the centre of the thorax and lungs.
        ...
        \subsection{Combined PET/CT} \label{sec:combined_pet_ct}
            ...
            \subsubsection{Attenuation Correction} \label{sec:attenuation_correction}
                ...
                \begin{figure}
                    ...
                    \captionsetup{singlelinecheck=false, justification=raggedright}
                    \caption{Example of simulated \glsed{NAC} \gls{TOF} data, with motion, with no noise randoms or scatters, of the thorax with a spherical lesion in the lungs. Transverse view.} \label{fig:combined_pet_ct_nac_tof_example}
                \end{figure}
...
%            \gls{BP} and \gls{FBP} reconstruct a low quality image but are computationally fast. \gls{BP} will usually result in there being numerous streak like artefacts in the output image, the artefacts are exacerbated as noise increases. Artefacts can be caused because neither \gls{BP} nor \gls{FBP} account for the stochastic nature of the acquisition process, neither do they account for other factors such as positron range as discussed in~\Fref{sec:attenuation}.
        ...
        \subsection{Iterative Image Reconstruction} \label{sec:iterative_image_reconstruction}
            \begin{figure}
                ...
                \captionsetup{singlelinecheck=false, justification=raggedright}
                \caption{Example of a simulated \glsed{AC} \gls{NTOF} \gls{OSEM} reconstruction, with motion and noise, with no randoms or scatters, of the thorax with a spherical lesion in the lungs. Transverse view.}
    ...
    \longsection{Respiratory Motion in PET}{sec:respiratory_motion_in_pet}
        ...
        \subsection{Respiratory Motion Artefacts} \label{sec:respiratory_motion_artefacts}
            ...
            \begin{figure}
                ...
                \captionsetup{singlelinecheck=false, justification=raggedright}
                \caption{Example of a simulated \glsed{AC} \gls{NTOF} \gls{OSEM} reconstruction, with motion, with no noise, randoms or scatters, of the thorax with a spherical lesion in the lungs. Coronal view.}
            ...
            \begin{figure}
                ...
                \captionsetup{singlelinecheck=false, justification=raggedright}
                \caption{Example of a simulated single \gls{Mu-Map} \glsed{AC} \gls{NTOF} \gls{OSEM} reconstruction, with motion, with no noise, randoms or scatters, of the thorax with a spherical lesion in the lungs. Coronal view.}
        ...
        \subsection{Motion Modelling} \label{sec:motion_modelling}
            ...
            \subsubsection{Respiratory Correspondence Model} \label{sec:respiratory_correspondence_model}
                ...
                \begin{itemize}
                    ...
                    \item Two, a dynamic image could be used as the static image. Here, usually the dynamic image with the highest number of counts, and hence the lowest amount of noise or best \gls{SNR}, is used. This static image is registered as discusses above, however it is never updated. In this instance the \gls{RCM} can be given an extra degree of freedom by increasing the dimensional of the \gls{SS}, hence allowing for the \gls{RCM} fitting to, ideally, find a transformation from the static image position to the mean position.
                \end{itemize}
